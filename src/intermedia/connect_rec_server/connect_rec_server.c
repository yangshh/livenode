...
int32_t deal_select_udp_array(udp_socket_channelID_info_t *socket_channelID,connect_server_info_t * connect_info_p, fd_set *fdsr, int32_t msg_data_socket , int32_t recv_socket)
{
	int32_t					udp_socket_loop = 0;
	int32_t 					live_recv_code = 0;
	int32_t					channel_id = socket_channelID->m_channelID ;
	int32_t					online = 0;
	
	for(udp_socket_loop = 0;udp_socket_loop<socket_channelID->m_num_udp_socket;udp_socket_loop++)
	{
		 live_recv_code = 0; 
		if (FD_ISSET(socket_channelID->udp_socket[udp_socket_loop], fdsr)) 
		{
			recv_socket = socket_channelID->udp_socket[udp_socket_loop];
			while(1)
			{
				#if RECV_UDPDATA_S
				printf("[%s]---channelID [%d]---index[%d] to call [live_rec_server_recv_data]!\n",__func__,channel_id,udp_socket_loop);
				#endif
				live_recv_code = live_rec_server_recv_data(msg_data_socket, recv_socket,udp_socket_loop,channel_id);
				if(live_recv_code == -1 ||live_recv_code ==0)
				{
					break;
				}
			}
	...
	if(online == 0 )
	{
		if(broadcast_udp_check_online(connect_info_p,channel_id) == -1)
		{							
			printf("[%s]---broad_ip [%s]---broad_id[%d]---local_id[%d] is error in the [tim_subtract]\n",__func__,connect_info_p->m_ip_addr,connect_info_p->m_channel,channel_id);
			return -1;
		}					
	}
	else
	{
		if(connect_info_p->time_udp_no_data.tv_sec != 0 ||connect_info_p->time_udp_no_data.tv_usec != 0)
		{
			connect_info_p->time_udp_no_data.tv_sec = 0;
			connect_info_p->time_udp_no_data.tv_usec = 0;
		}
	...
	const int32_t channel,const int8_t* server_ip,int32_t socket, int32_t udp_start_port)
{
	...
	temp = channel;
	r_memcpy(buf + 12,&temp,4);

	serv_addr.sin_family = AF_INET;
	serv_addr.sin_port = r_htons(port);
	r_inet_aton(server_ip,(struct in_addr *)&serv_addr.sin_addr);
	r_bzero(&(serv_addr.sin_zero),8);
	send_len = r_sendto(socket,  buf,  16, 0, (struct sockaddr*)&serv_addr,sizeof(struct sockaddr_in));
#endif
...
static int32_t request_media_data(int32_t socket, int32_t channel)
{
	...
	test = 1;
	r_memcpy(data + 4,&test,1);
	
	test = 1;
	r_memcpy(data + 5,&test,1);
	test = 1;
	r_memcpy(data + 6,&test,1);	
	test = 1;
	r_memcpy(data + 7,&test,1);
...
static void* create_msg_data_thread(void* arg)
{
	...
	while(1)
	{
		...
		recv_socket = connect_info[0].m_udp_socket[0];
		
		FD_SET(recv_socket, &fdsr);	
		select_recode = select(recv_socket + 1, &fdsr, NULL, NULL, &tv);
		...
		default:
			{
				if(FD_ISSET(recv_socket, &fdsr))
				{
					while(1)
					{
						...
						printf("[%s]---channelID [%d]---index[%d] to call [live_rec_server_recv_data]!\n",__func__,channel_id,udp_socket_loop);
						#endif
						live_recv_code = live_rec_server_recv_data(msg_data_socket, recv_socket,0,0);
						if(live_recv_code == -1 ||live_recv_code ==0)
						{
							break;
						}
	...
	{		
		...
		switch(select_recode)
		{
			...
			case 0:
			{
				...
				for( z_loop = 0; z_loop<CHANNEL_CONNECT_MAX_NUM;z_loop++)
				{
					if(socket_channelID[z_loop].m_channelID != -1)
					{	
						#if RECV_SELECT_S
						printf("[%s]----[%d]----[%d] is select timeout\n",connect_info[(socket_channelID[z_loop].m_channelID)].time_udp_no_data.tv_sec ,connect_info[(socket_channelID[z_loop].m_channelID)].time_udp_no_data.tv_usec);
						#endif
						if(broadcast_udp_check_online(&connect_info[(socket_channelID[z_loop].m_channelID)],socket_channelID[z_loop].m_channelID) == -1)
						{
			...
			default:
			{
				for(socket_channelID_loop=0;socket_channelID_loop<CHANNEL_CONNECT_MAX_NUM;socket_channelID_loop++)
				{
					...
					if(deal_select_udp_array(&socket_channelID[socket_channelID_loop],&connect_info[channel_id],&fdsr,msg_data_socket,recv_socket) != 0)
					{
						printf("[%s]---[deal_select_udp_array] is error!\n",__func__);
					}
