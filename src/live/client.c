...
int32_t local_recv_process(user_t *u)
{
	...
	while(1) {
		...
		if(ret < 0) {
			if(-1 == ret) {
				printf("[local_recv_process] --------------- recv failed !!!\n");
				close_client(u);
			}
		...
		if(MAX_LIST_LEN < list_len) {
			...
			for(index = 0; index < MAX_USERS; index++) {
				if(get_user_node(index) == head) {
					...
					printf("[local_recv_process] --------------- list_len:[%d] > max : [%d]\n", list_len, MAX_LIST_LEN);
...
int32_t send_client_video_data(user_t *u)
{
	...
	while(1) {
		...
		if(ret < 0) {
			...
			if(-1 == ret) {
				printf("[send_client_video_data] ----------------- send failed !!!\n");
				close_client(u);
				break;
			} else {
				/*计算这次事件发送了多少数据*/
...
int32_t net_recv_process(user_t *u)
{
	...
	while(1) {
		...
		if(ret < 0) {
			...
			if(-1 == ret) {
				printf("[net_recv_process] -------------recv head failed !!\n");
				close_client(u);
			}
		...
		if(ret < 0) {
			...
			if(-1 == ret) {
				printf("[net_recv_process] -------------recv remain failed !!\n");
				close_client(u);
			}
		...
		if(-1 == u->m_is_send || -1 == ret) {
			printf("[net_recv_process] -------------send  failed !!\n");
			close_client(u);
			return ret;
		}
	}
